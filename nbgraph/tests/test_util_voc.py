...
from nbgraph.config import FileWord2Vec, FileVoc, FileWS, TestFWord2Vec, TestFVoc, TestFWS,\
    TestFWSChildParent, TestFWSParentChildren, TestFWSPaths, FileWSChildParent, FileWSParentChildren, FileWSPaths
from nbgraph.util_voc import create_vocabulary, get_voc_list, create_word_sense_file, create_ws_struc_txts
import unittest 
...
class VocTest(unittest.TestCase):
    ...
    def test_ws_struc_txts(self):
        self.assertEqual(create_ws_struc_txts(wsFile=FileWS, vocFile=FileVoc,
                             hypernymsFile=FileWSChildParent, hyponymsFile=FileWSParentChildren,
                             pathsFile=FileWSPaths), 0)
